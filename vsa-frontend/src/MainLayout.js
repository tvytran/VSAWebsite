...
function MainLayout({ children }) {
  ...
  if (isGuest) {
    return (
      <div className="min-h-screen w-full bg-[#faecd8]">
        {/* Simple top navigation for guests */}
        <nav className="bg-white shadow-md">
          <div className="max-w-6xl mx-auto px-4">
            <div className="flex justify-between items-center h-16">
              <div className="flex space-x-4">
                <Link to="/families" className="text-gray-700 hover:text-[#b32a2a]">Leaderboard</Link>
                <Link to="/about" className="text-gray-700 hover:text-[#b32a2a]">About VSA</Link>
                <button 
                  onClick={() => {
                    localStorage.removeItem('isGuest');
                    window.location.href = '/';
                  }} 
                  className="text-gray-700 hover:text-[#b32a2a]"
                >
                  Exit Guest Mode
                </button>
              </div>
    ...
    <div className="min-h-screen w-full bg-[#faecd8] flex flex-col">
      <div className="flex flex-1">
        ...
        <div className="w-72 bg-[#faecd8] flex flex-col items-center py-8 border-l border-[#e0c9a6] min-h-screen">
          <Link to="/create-post" className="w-48 py-4 mb-8 bg-[#b32a2a] text-white text-xl font-semibold rounded-md hover:bg-[#8a1f1f] flex items-center justify-center transition duration-200 ease-in-out">Create</Link>
